...
public:
    ...
    int GetDatabyTerminalAddrAndTime(DATA &dataA, DATA &dataB, DATA &dataC, int TerminalAddrA, int TerminalAddrB, int TerminalAddrC, INT64 time, int conAddr);
	...
	//获得数据表中的时间，并以集中器分组
    int GetCollectTimeAndMoniterID(vector<TIME_ID> &v, int ConcentratorAddr);
    int GetCollectTimeAndMoniterID(vector<TIME_ID> &v, int ConcentratorAddr, INT64 begin, INT64 end);
	//获得数据表中的时间，并以集中器分组
